...
func main() {
     ...
     f, err := excelize.OpenFile("crony_go.xlsx")  // open Excel file
     if err != nil {
        panic(err)
     }
    ...
    err = f.SaveAs("./crony_go.xlsx")  // save the excel with data
     if err != nil {
        fmt.Println(err)
     }
...
//        panic("Failed to Dial: "+ err.Error())
     ...
     var c bytes.Buffer
     session2.Stdout = &c
     if err := session2.Run(" chronyc sourcestats | awk 'FNR == 4 {print}' | awk '{print $7}' "); err != nil {
        log.Println("Failed to run on client: " + err.Error())
        return  "", "", err
     }
